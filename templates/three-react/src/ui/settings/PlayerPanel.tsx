import { useGameStore, type MovementParams, type MovementMode } from '../../store';
import { SettingsWindow, Section, Slider, Toggle, type SliderDef, resetBtnStyle, rowStyle } from './shared';
  ...
  return (
    <SettingsWindow>
      <Section label="Move" first>
        ...
        <Toggle label="Path Debug" value={characterParams.showPathDebug} onChange={(v) => setCharacterParam('showPathDebug', v)} />
        <Toggle label="Character push" value={characterPushEnabled} onChange={setCharacterPushEnabled} />
        <Toggle label="Foot IK" value={characterParams.footIKEnabled} onChange={(v) => setCharacterParam('footIKEnabled', v)} />
        {CHARACTER_MOVE_PARAMS.map(({ key, label, min, max, step }) => (
          <Slider key={key} label={label} value={characterParams[key] as number} min={min} max={max} step={step}
            onChange={(v) => setCharacterParam(key, v as any)} />
        ))}
      </Section>

      <Section label="Defense">
        <Slider label="Invuln" value={characterParams.invulnDuration} min={0} max={2} step={0.05}
          onChange={(v) => setCharacterParam('invulnDuration', v)} />
        <Slider label="Stun" value={characterParams.stunDuration} min={0} max={1} step={0.05}
          onChange={(v) => setCharacterParam('stunDuration', v)} />
      </Section>

      <Section label="Melee">
        <Toggle label="Auto-target" value={characterParams.melee.autoTarget} onChange={(v) => setMeleeParam('autoTarget', v)} />
        <Slider label="Knockback" value={characterParams.melee.knockback} min={0} max={15} step={0.5}
          onChange={(v) => setMeleeParam('knockback', v)} />
        <Toggle label="Slash effect" value={characterParams.melee.showSlashEffect} onChange={(v) => setMeleeParam('showSlashEffect', v)} />
        <Toggle label="Exhaustion" value={characterParams.melee.exhaustionEnabled} onChange={(v) => setMeleeParam('exhaustionEnabled', v)} />
      </Section>

      <Section label="Ranged">
        <Toggle label="Auto-target" value={characterParams.ranged.autoTarget} onChange={(v) => setRangedParam('autoTarget', v)} />
        <Slider label="Knockback" value={characterParams.ranged.knockback} min={0} max={15} step={0.5}
          onChange={(v) => setRangedParam('knockback', v)} />
        <Toggle label="Exhaustion" value={characterParams.ranged.exhaustionEnabled} onChange={(v) => setRangedParam('exhaustionEnabled', v)} />
        <Toggle label="Debug" value={debugProjectileStick} onChange={setDebugProjectileStick} />
      </Section>
