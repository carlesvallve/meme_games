...
import { SettingsWindow, Section, Slider, CollisionLayerSelect, type SliderDef, resetBtnStyle } from './shared';
    ...
    <SettingsWindow>
      <Section label="Camera" first>
        {CAMERA_PARAMS.map(({ key, label, min, max, step }) => (
          <Slider key={key} label={label} value={cameraParams[key] as number} min={min} max={max} step={step}
