...
export class CharacterModel {
  ...
  constructor(opts: CharacterModelOpts) {
    ...
    loader.load(opts.meshUrl, (gltf) => {
      ...
      // Compute T-pose ground offset once from rest pose
      this.computeRestPoseOffset(model);

      this.mixer = new THREE.AnimationMixer(model);

      // Debug: dump bone names from mesh skeleton
      const boneNames: string[] = [];
      model.traverse((child) => {
        if ((child as THREE.Bone).isBone) boneNames.push(child.name);
      });
      console.log(`[CharacterModel] Mesh bones (${boneNames.length}):`, boneNames.slice(0, 15));
      ...
      model.traverse((child) => {
        if ((child as THREE.Mesh).isMesh) {
          meshCount++;
          vertCount += (child as THREE.Mesh).geometry.getAttribute('position')?.count ?? 0;
        }
      });
      const box = new THREE.Box3().setFromObject(model);
      const size = box.getSize(new THREE.Vector3());
      console.log(`[CharacterModel] Mesh: ${meshCount} meshes, ${vertCount} verts, bounds: (${size.x.toFixed(3)}, ${size.y.toFixed(3)}, ${size.z.toFixed(3)}), pos.y=${model.position.y.toFixed(4)}`);
  ...
  /** Compute ground offset from T-pose (rest pose) — called once at load time.
   *  Finds the lowest bone world Y and adjusts offsetY so feet sit at ground level. */
  private computeRestPoseOffset(model: THREE.Object3D): void {
    ...
    // Force world matrix update for the rest pose
    ...
    if (minY < Infinity) {
      ...
      model.position.y = this.offsetY;
      console.log(`[CharacterModel] T-pose ground offset: ${correction.toFixed(4)}`);
    }
  }
