...
function voxBox(w: number, h: number, d: number, x: number, y: number, z: number, hex: number): THREE.BufferGeometry {
...
function tintHex(hex: number, factor: number): number {
  const r = Math.min(255, Math.max(0, ((hex >> 16) & 0xff) + Math.round(factor * 255)));
  const g = Math.min(255, Math.max(0, ((hex >> 8) & 0xff) + Math.round(factor * 255)));
...
function buildDeciduousTree(leafColor: number, trunkColor: number, rng: () => number): THREE.BufferGeometry {
...
function buildPineTree(leafColor: number, trunkColor: number, rng: () => number): THREE.BufferGeometry {
  const trunkW = V * 1;
...
function buildPalmTree(leafColor: number, trunkColor: number, rng: () => number): THREE.BufferGeometry {
  ...
  for (let i = 0; i < frondCount; i++) {
    ...
    const frondW = V * 2;
    const frond = voxBox(frondLen, V, frondW, 0, 0, 0, tintHex(leafColor, (rng() - 0.5) * 0.06));
    frond.rotateY(angle);
    frond.rotateX(-0.35);
    frond.translate(cx + Math.cos(angle) * V * 2, topY, Math.sin(angle) * V * 2);
    parts.push(frond);
  }
...
function buildSimpleCubeTree(leafColor: number, trunkColor: number, rng: () => number): THREE.BufferGeometry {
  const trunkW = V * (0.8 + rng() * 0.5);
...
function buildCactus(leafColor: number, _trunkColor: number, rng: () => number): THREE.BufferGeometry {
  ...
  for (let i = 0; i < armCount; i++) {
    ...
    parts.push(voxBox(horizLen, armW, armW, side * (bodyW / 2 + horizLen / 2), yOff, 0, shade1));
...
function buildGrassTuft(color: number, rng: () => number): THREE.BufferGeometry {
  const parts: THREE.BufferGeometry[] = [];
...
function buildFlower(petalColor: number, stemColor: number, rng: () => number): THREE.BufferGeometry {
  ...
  if (rng() > 0.4) {
    ...
    for (const [dx, dz] of picked) {
      ...
      parts.push(voxBox(petalW, petalW, petalW, dx * headSize * 0.7, stemH + headSize * 0.1, dz * headSize * 0.7, shade));
    }
  }
...
function displaceVertices(geo: THREE.BufferGeometry, amount: number, rng: () => number): void {
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    pos.setXYZ(i, nx + (nx / len) * d, ny + (ny / len) * d, nz + (nz / len) * d);
  }
  pos.needsUpdate = true;
  geo.computeVertexNormals();
}

function buildOrganicDeciduousTree(leafColor: number, trunkColor: number, rng: () => number): THREE.BufferGeometry {
...
function buildOrganicPineTree(leafColor: number, trunkColor: number, rng: () => number): THREE.BufferGeometry {
...
function buildOrganicPalmTree(leafColor: number, trunkColor: number, rng: () => number): THREE.BufferGeometry {
  ...
  for (let i = 0; i < frondCount; i++) {
    ...
    frond.rotateY(angle);
    frond.translate(Math.cos(angle) * 0.05, trunkH - 0.02, Math.sin(angle) * 0.05);
...
function buildOrganicGrassTuft(color: number, rng: () => number): THREE.BufferGeometry {
...
function buildOrganicFlower(petalColor: number, stemColor: number, rng: () => number): THREE.BufferGeometry {
...
function createBatch(geo: THREE.BufferGeometry, maxCount: number, castShadow = false): InstancedBatch {
...
function addInstance(
  batch: InstancedBatch, x: number, y: number, z: number,
  rotY: number, scale: number, tiltX = 0, tiltZ = 0,
): void {
...
function addInstanceOnNormal(
  batch: InstancedBatch, x: number, y: number, z: number,
  normal: THREE.Vector3, rotY: number, scale: number,
): void {
  if (batch.count >= batch.maxCount) return;
  _tmpPos.set(x, y, z);
...
export function generateNature(
  ...
  for (let v = 0; v < ROCK_VARIANTS; v++) {
    rockGeos.push(buildRock(preset.rockColor, mulberry32(seed + 2222 + v * 333)));
  }

  const grassGeos: THREE.BufferGeometry[] = [];
  for (let v = 0; v < GRASS_VARIANTS; v++) {
    const gc = preset.grassColors.length > 0
      ? preset.grassColors[v % preset.grassColors.length] : 0x448844;
    grassGeos.push(buildGrassTuft(hueShift(gc, mulberry32(seed + 7777 + v), 0.08), mulberry32(seed + 3333 + v * 444)));
  }
  ...
  if (preset.flowerColors.length > 0) {
    for (let v = 0; v < FLOWER_VARIANTS; v++) {
      ...
      flowerGeos.push(buildFlower(hueShift(fc, mulberry32(seed + 8888 + v), 0.12), 0x2a6a20, mulberry32(seed + 4444 + v * 555)));
    }
  }

  // Create batches with generous limits
  const treeBatches: InstancedBatch[] = treeGeos.map(g => createBatch(g, 200, true));
  const rockBatches: InstancedBatch[] = rockGeos.map(g => createBatch(g, 150));
  const grassBatches: InstancedBatch[] = grassGeos.map(g => createBatch(g, 500));
  const flowerBatches: InstancedBatch[] = flowerGeos.map(g => createBatch(g, 200));
  ...
  const PATCH_THRESHOLD = 0.70;

  const treePatch = (x: number, z: number) => fbmNoise(x * patchFreq, z * patchFreq, seed + 7001, 2);
  const rockPatch = (x: number, z: number) => fbmNoise(x * patchFreq * 0.85, z * patchFreq * 0.85, seed + 7002, 2);
  const flowerPatchNoise = (x: number, z: number) => fbmNoise(x * patchFreq * 1.1, z * patchFreq * 1.1, seed + 7003, 2);
  ...
  for (let wx = -halfGround + margin; wx < halfGround - margin; wx += sampleStep) {
    for (let wz = -halfGround + margin; wz < halfGround - margin; wz += sampleStep) {
      ...
      if (treeInPatch && slope < maxSlopeForTrees && preset.treeDensity > 0 && treeBatches.length > 0) {
        if (rng() < treeFill) {
          ...
          addInstance(treeBatches[batchIdx], jx, h, jz, rotY, scale, tiltX, tiltZ);
      ...
      if (rockInPatch && preset.rockDensity > 0 && rockBatches.length > 0) {
        if (rng() < rockFill) {
          ...
          if (rng() < 0.7) {
            ...
            addInstanceOnNormal(rockBatches[batchIdx], jx, h - sink, jz, normal, rotY, scale);
          } else {
            const tiltX = (rng() - 0.5) * 0.6;
            const tiltZ = (rng() - 0.5) * 0.6;
            addInstance(rockBatches[batchIdx], jx, h - sink * 1.5, jz, rotY, scale, tiltX, tiltZ);
          }
        }
      }
      ...
      if (flowerInPatch && slope < maxSlopeForFlowers && preset.flowerDensity > 0 && flowerBatches.length > 0) {
        if (rng() < flowerFill) {
      ...
      if (useBiomes && preset.scatterTreeChance && tp <= PATCH_THRESHOLD && slope < maxSlopeForTrees && treeBatches.length > 0) {
      ...
      if (useBiomes && rp <= PATCH_THRESHOLD && rockBatches.length > 0 && rng() < 0.015) {
        const batchIdx = Math.floor(rng() * rockBatches.length);
        const scale = 0.4 + rng() * 0.8;
        const sink = V * scale * 0.4;
        const tiltX = (rng() - 0.5) * 0.4;
        const tiltZ = (rng() - 0.5) * 0.4;
        addInstance(rockBatches[batchIdx], jx, h - sink, jz, rotY, scale, tiltX, tiltZ);
      }
    }
  }

  // Finalize instanced meshes
  const allBatches = [...treeBatches, ...rockBatches, ...grassBatches, ...flowerBatches];
